...
def plot_learning_curve(x, scores, epsilon, filename):
    ...
    # Plot running average of scores
    ax1.plot(x, running_avg, color="C1", label='Score (running avg)')
    ax1.set_xlabel("Training iterations")
    ax1.set_ylabel("Score", color="C1")
    ax1.tick_params(axis='y', colors="C1")
    
    # Add vertical line when epsilon equals 0.1
    epsilon_threshold = 0.1
    epsilon_steps = [i for i, eps in enumerate(epsilon) if eps <= epsilon_threshold]
    
    if epsilon_steps:
        first_threshold_step = x[epsilon_steps[0]]
        ax1.axvline(first_threshold_step, color='r', linestyle='--', label='Epsilon = 0.1')
